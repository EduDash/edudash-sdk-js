...
export class SdkClientTocPlugin extends RendererComponent {
  ...
  private onRendererBeginPage(page: PageEvent) {
    ...
    if (!model.parent && !trail.length) {
      this.clientsNavigationItem = new NavigationItem(
        'Clients',
        void 0,
        page.toc
      );
      ...
      this.paginatorsNavigationItem = new NavigationItem(
        'Paginators',
        void 0,
        page.toc
      );
    ...
    return (
      ...
      (model.name.endsWith('Client') /* Modular client like S3Client */ ||
        (extendedTypes.length === 1 &&
          (extendedTypes[0] as ReferenceType).name.endsWith(
            'Client'
          ))) /* Legacy client like S3 */
    );
  }
  ...
  private isPaginator(model: DeclarationReflection): boolean {
    return (
      model.name.startsWith('paginate') && model.kindOf(ReflectionKind.Function)
    );
  }
  ...
  buildToc(
    ...
    if (index < trail.length - 1 && children.length > 40) {
      ...
      this.commandsNavigationItem?.children?.sort((childA, childB) =>
        childA.title.localeCompare(childB.title)
      );
    }
  }
}
